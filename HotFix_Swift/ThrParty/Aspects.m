...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    NSCParameterAssert(self);
    // self 对象的 isa 指针，就是当前类（类 ViewController 的地址）
    // 对象的isa指针指向当前类
	Class statedClass = self.class;
    // 当前类的 isa 指针（类ViewController 的 isa 地址）
	...
	if ([className hasSuffix:AspectsSubclassSuffix]) {
        ...
        // 判断是不是元类，元类的话直接调用 aspect_swizzleClassInPlace
	}else if (class_isMetaClass(baseClass)) {
        return aspect_swizzleClassInPlace((Class)self);
        // Probably a KVO'ed class. Swizzle in place. Also swizzle meta classes in place.
        // 到这里既不是元类也不是hook后的类可能是该类被kvo过，因为kvo会生成中间类
        // 被kvo过的对象因为对象的isa指针指向一个中间类，调用aspect_swizzleClassInPlace
    }else if (statedClass != baseClass) {
        return aspect_swizzleClassInPlace(baseClass);
    }

    // Default case. Create dynamic subclass.
    // 如果没有被hook过，不是元类，没有被kvo过，动态生成子类。
    // 动态生成子类，子类名字为：_Aspects_+className
	...
	if (subclass == nil) {
        ...
        // 子类生成后交换子类的 forwardInvocation 方法
...
static void aspect_swizzleForwardInvocation(Class klass) {
    ...
    // 更改IMP指针
    // 将系统 forwardInvocation 方法替换为自定义的 __ASPECTS_ARE_BEING_CALLED__ 方法
    // 这样可以使调用 forwardInvocation 方法时调用自定义的 __ASPECTS_ARE_BEING_CALLED__ 方法
    // class_replaceMethod 返回的是原方法对应的IMP，如果原来方法没有实现就返回nil。
    // 调用了 class_replaceMethod 会把原来方法的IMP指向另一个方法的实现，这里相当于添加了 forwardInvocation 方法，并且指向了 __ASPECTS_ARE_BEING_CALLED__
    IMP originalImplementation = class_replaceMethod(klass, @selector(forwardInvocation:), (IMP)__ASPECTS_ARE_BEING_CALLED__, "v@:@");
    // class_replaceMethod 源码底层也是调用了 class_addMethod 方法
